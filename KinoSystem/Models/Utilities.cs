...
namespace KinoSystem.Models.Utilities
{
    ...
    public class Utililies
    {
        ...
        public async static Task LoadMovies(KinoDBContext db)
        {

            if (db.Films.Count() > 0)
                return;

            using (var stream = new StreamReader("movies.txt"))
            {
           ...
           SetMoviesInRenatalAsync(db, 8);

        }
        public async static Task SetMoviesInRenatalAsync(KinoDBContext db, int count)
        {

            Random rnd = new Random();
            for (int i = 0; i < count; i++)
            {
                var num = rnd.Next(db.Films.Count()) - 1;
                var movie = await GetMovieByIdAsync(db, num);
                if (movie == null)
                    continue;
                movie.InRental = true;
                db.Update(movie);
